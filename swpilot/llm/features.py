...
CATALOG: tuple[CadFeature, ...] = (
    ...
    CadFeature("loft", "loft", "نفخ بين مقاطع",
               (r"\bloft(ed)?\b", r"نفخ بين مقاطع", r"ربط مقاطع"),
    ...
    CadFeature("draft", "draft angle", "زاوية سحب القالب",
               (r"\bdraft angle\b", r"\bdraft(ed)?\b", r"زاوية سحب", r"ميلان قالب"),
               "extrude's draft_angle field for simple tapers",
               "حقل draft_angle في البثق للميلان البسيط"),
    CadFeature("sheet_metal", "sheet metal (bends/flanges)", "صاج ومجلفن (ثني)",
               (r"\bsheet ?metal\b", r"\bbend(s|ing)?\b", r"\bflange\b", r"صاج", r"ثني معدن"),
    ...
    CadFeature("worm_gear", "worm gear", "ترس دودي",
               (r"\bworm( gear)?\b", r"ترس دودي", r"دودة و ترس"),
